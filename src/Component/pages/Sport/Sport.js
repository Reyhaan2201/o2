...
function Sport() {
 ...
 const [text, settext] = useState("Favourites");
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:1804/Outfitoracle/${Productcategory}/Sport`)
      .then((res) => {
        console.log("Successfully got the Sport product category", res.data);
        setProduct(res.data);
      })
      .catch((err) => {
        console.log("Error getting the product category of Sport", err.message);
      });
  }, [Productcategory]);
